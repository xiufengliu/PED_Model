...
def create_network(config_file, params_file, data_path):
    ...
    area_m2 = pv_params["surface_m2"] # Questo sarà 684 m² (high_pv configuration)
    ...
    # Evita arbitraggio: prezzi non negativi e export_price = import_price
    ...
    # Ricavo export: se il prezzo è 0, applica un epsilon negativo per rompere il pareggio col curtailment
    eps = 1e-6  # usa le stesse unità della tua serie prezzi
    _ser = pd.Series(export_price_series, index=network.snapshots)
    mc_export = -(_ser.where(_ser > 0, eps))
    network.links_t.marginal_cost.loc[:, "PV Export"] = mc_export.to_numpy()
