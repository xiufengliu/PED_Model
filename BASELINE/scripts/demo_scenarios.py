...
def main():
    ...
    high_pv_output_dir = os.path.join(base_dir, 'data', 'output', 'high_pv_social_building')
    ...
    import importlib
    print("[DEBUG] Importing scenarios.high_pv...")
    high_pv_mod = importlib.import_module('scenarios.high_pv')
    print("[DEBUG] Calling simulate_high_pv...")
    high_pv_results = high_pv_mod.simulate_high_pv(params, profiles, n_hours)
    print("[DEBUG] simulate_high_pv completed!")

    # Create visualizations and save summary for high_pv
    if hasattr(high_pv_mod, 'create_visualizations'):
        high_pv_summary = high_pv_mod.create_visualizations(high_pv_results, high_pv_output_dir)
    else:
        # Use baseline visualizations if not present
        high_pv_summary = create_visualizations(high_pv_results, high_pv_output_dir)

    print("\nHigh PV Scenario Summary:")
    print("========================")
    for i, row in high_pv_summary.iterrows():
        print(f"{row['Metric']}: {row['Value']:.2f}" if isinstance(row['Value'], float) else f"{row['Metric']}: {row['Value']}")
