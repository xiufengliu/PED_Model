...
def save_hourly_results(network, output_dir):
    """
    Estrae e salva i risultati orari coerenti con lo schema attuale
    (HP solo PV, TES Store+Link, flussi netti con -p1).
    Ritorna un DataFrame con una colonna per ciascuna serie.
    """
    import os
    import pandas as pd
    ...
    # Flussi elettrici (MW; con Δt=1h, la somma dà MWh)
    ...
    th_store_dispatch      = ( network.links_t.p0.get("Thermal Dispatch", z())).clip(lower=0.0)

    # Batteria: SoC e flussi netti
    ...
    # Costruzione DataFrame (colonne coerenti col main)
    df_hourly = pd.DataFrame({
        ...
        "th_store_dispatch":         th_store_dispatch.values,
...
def compute_pv_battery_breakdown(net):
    """
    Totali [MWh]:
      • Grid Import (post-perdite, lato building)
      • PV → Building Elec            (link "PV Autoconsumo")
      • PV → Battery Storage          (link "Battery Charge")
      • Battery → Building Elec       (link "Battery Dispatch")
      • PV → Heat Source (Heat Pump)  (link "Heat Pump", lato bus0)
    """
    import pandas as pd
    Z = pd.Series(0.0, index=net.snapshots)

    # Import al building (post-perdite): -p1 su "Grid Import"
    grid_imp = (-net.links_t.p1.get('Grid Import', Z)).clip(lower=0.0)

    pv_to_load   =  net.links_t.p0.get('PV Autoconsumo',  Z)
    pv_to_batt   =  net.links_t.p0.get('Battery Charge',  Z)
    batt_to_load =  net.links_t.p0.get('Battery Dispatch',Z)
    pv_to_hp     =  net.links_t.p0.get('Heat Pump',       Z)
