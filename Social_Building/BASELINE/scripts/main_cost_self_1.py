...
def calculate_summary(network) -> pd.DataFrame:
    ...
    imp_storage = network.links_t.p0.get('Grid Import_Storage', pd.Series(0.0, index=network.snapshots))
    # Somma dei due flussi orari dalla rete
    imp = imp_link + imp_storage
    ...
    ε7 = network.epsilon_grid_storage

    # 3’) Calcolo i costi orari applicando gli overhead originali
    cost_grid_import  = (imp_link    * (grid_price + ε6) * w).sum()
    cost_grid_storage = (imp_storage * (grid_price + ε7) * w).sum()
    total_grid_import_cost = cost_grid_import + cost_grid_storage
    ...
    print("DEBUG: mc_store_link =", network.links.at['Grid Import_Storage','marginal_cost'])

    return pd.DataFrame([{
        'Total Grid Import (MWh)':        total_imp,
        'Total Grid Export (MWh)':        total_exp,
        'Grid Import Cost (EUR)':         cost_grid_import,
        'Grid→Storage Cost (EUR)':        cost_grid_storage,
