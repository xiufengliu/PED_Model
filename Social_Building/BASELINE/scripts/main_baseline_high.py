...
def calculate_summary(network) -> pd.DataFrame:
    ...
    # Prezzo DH a valle (EUR/MWh_th):
    # 1) se esiste serie su link, usa quella;
    # 2) altrimenti prendi prezzo del generatore "Heating" (serie o scalare) e dividi per eta_dh.
    if dh_link and hasattr(network.links_t, "marginal_cost") and dh_link in Lmc_t.columns:
        dh_price_net = Lmc_t[dh_link].reindex(snaps).fillna(0.0).astype(float)
    else:
        ...
        # baseline imposta costo_gen = costo_consegna × eta_dh  → costo_consegna = costo_gen / eta_dh
        dh_price_net = (dh_price_gen / max(eta_dh, 1e-12)).astype(float)

    heat_cost_dh = float((heat_dh_delivered * dh_price_net * w).sum())
