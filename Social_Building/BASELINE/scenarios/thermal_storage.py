def create_network(config_file, params_file, data_path):
    """
    Scenario PV + Battery + Thermal Storage (allineato a logica PV-first):
      Ordine di priorità (ε: più negativo = più prioritario)
        1) PV → Heat Pump → Building Heat
        2) PV → Building Elec (autoconsumo)
        3) PV → Heat Pump → Thermal Storage (carica TES)
        4) PV → Battery (carica)
        5) PV → Export (solo da PV; costo dinamico = −prezzo + micro-offset)

      Dispatch (solo quando serve, con piccolo costo positivo per evitare cicli):
        • Thermal Storage → Building Heat   (+2e-4)
        • Battery → Building Elec           (+2e-4)

      Ultime risorse (governate da prezzi reali, nessun ε):
        • Grid Import (Generator "Grid")
    ...
    # -----------------------------
    # ε per imporre le priorità (più negativo = più prioritario)
    # -----------------------------
    ...
    eps = {
        # Termico (PV → HP → Building prima di qualunque accumulo)
        "hp":               -10.0*BASE,  # input elettrico alla HP
        "th_discharge":     -9.0*BASE,   # Heat Source → Building Heat (calore ora)
        # Elettrico diretto prima del TES
        "pv_to_building":   -7.0*BASE,   # PV → Building Elec
        # Solo dopo: carica TES e poi batteria
        "th_charge":        -5.0*BASE,   # Heat Source → Thermal Storage (carica)
        "pv_to_batt":       -4.0*BASE,   # PV → Battery (carica)
        # Export (micro-offset)
    ...
    # -----------------------------
    # Heat Pump (SOLO PV → calore su Heat Source Bus)
    ...
    # 1) HP→Building Heat (prima della carica TES)
    ...
    # 3) HP→TES (carica) dopo aver servito l'edificio e PV→Building
    ...
    # Dispatch termico (solo quando serve): piccolo costo positivo
    ...
    # ELETTRICO (PV-first)
    # -----------------------------
    # 2) PV → Building (autoconsumo) – prima del TES
    ...
    # Dispatch elettrico (solo quando serve): piccolo costo positivo
    ...
    # Export Sink: assorbe l'energia esportata (nessun rientro)
