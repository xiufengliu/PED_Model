def create_network(config_file, params_file, data_path):
    ...
    if float(pd.Series(grid_price_series).mean()) < 10.0:
        print("⚠️ Prezzi rilevati probabilmente in EUR/kWh → converto a EUR/MWh (×1000).")
        grid_price_series = grid_price_series * 1000.0
