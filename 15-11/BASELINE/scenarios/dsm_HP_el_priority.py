def create_network(config_file, params_file, data_path):
    """
    DSM con micro-priorità (ε) orientate all'ELETTRICO:
      Ordine: ELETTRICO -> TERMICO -> EXPORT
      - Mantiene i nomi legacy (PV Autoconsumo, Thermal Discharge, Battery Store, ...)
      - Struttura identica allo scenario allegato
    """
    import os
    import pypsa
    import pandas as pd
    ...
    # COP(t) Heat Pump: Carnot realistico
    eta_carnot = 0.50
    T_sink_C   = 50.0
    ...
    # -----------------------------
    # Micro-priorità (ε) — ELECTRIC FIRST
    # -----------------------------
    # ELETTRICO (più bassi => più prioritari)
    EPS_EL_AUTOCONS = -1.0e-4   # PV → Building (PV Autoconsumo)
    EPS_EL_BAT_DIS  = -9.0e-5   # Battery → Building
    EPS_EL_DSM_CH   = -8.0e-5   # Building → DSM Elec Store
    EPS_EL_BAT_CH   = -7.0e-5   # PV → Battery

    # HEAT PUMP (dietro all’elettrico ma comunque premiata vs termico puro)
    EPS_HP          = -5.0e-5   # Building Elec → Heat Source Bus (Heat Pump)

    # TERMICO (meno premiato → meno negativo / zero)
    EPS_TH_DIRECT   = -3.0e-5   # Heat Source → Building (Thermal Discharge)
    EPS_TH_TES_DIS  = -2.0e-5   # TES → Building (Thermal Dispatch)
    EPS_TH_DSM_CH   = -1.0e-5   # Heat Source → DSM Heat Store (DSM Heat Charge)
    EPS_TH_TES_CH   = -1.0e-6   # Heat Source → TES (Thermal Charge)

    # EXPORT (sfavorito → piccolo costo positivo)
    EPS_EXPORT      =  -1.0e-7

    # -----------------------------
    # TERMICO (con priorità dietro l'elettrico)
    # -----------------------------
    n.add("Link", "Heat Pump",
          bus0="Building Elec", bus1="Heat Source Bus",
          efficiency=1,
          p_nom=heat_capacity*3, p_nom_extendable=False,
          marginal_cost=EPS_HP, carrier="heat_pump")
    ...
    # anti-KeyError: e_nom > 0 e nome esatto "Battery Store"
    ...
    print("links:", list(n.links.index)[:12], "... tot:", len(n.links.index))
    ...
    # Compatibilità report/validator
