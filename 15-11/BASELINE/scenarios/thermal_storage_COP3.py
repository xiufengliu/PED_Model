def create_network(config_file, params_file, data_path):
    """
    Scenario PV + Battery + Thermal Storage — PRIORITÀ: elettrico -> poi termico
    Scelte chiave:
      • Heat Pump alimentata SOLO da PV (bus0="PV Bus"): non può usare la rete => nessun import extra.
      • Costi marginali orari (benefici) che impongono l'ordine sul PV:
          1) PV→Building      = - grid_price(t) - 1e-4
          2) PV→Battery       = - 0.5*grid_price(t) - 8e-6
          3) PV→Heat Pump     = - 0.25*grid_price(t) - 5e-6
          4) PV→Export        = - export_price(t) - 1e-6
      • HP con p_nom FINITO (non estendibile), così non “aspira” tutto il PV.
      • Dispatch Battery/TES con piccolo costo positivo per evitare cicli.
    """
    import os
    import pypsa
    import pandas as pd
