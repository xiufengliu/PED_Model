def create_network(config_file, params_file, data_path):
    """
    DSM con micro-priorità (ε):
      Priorità: Heat Pump (HP) -> Termico -> Elettrico -> Export
      - Mantiene i nomi legacy (PV Autoconsumo, Thermal Discharge, Battery Store, ...)
      - Non cambia la struttura DSM originale (store + load sul bus DSM)
    """
    import os
    import pypsa
    import pandas as pd
    ...
    # DSM (domanda flessibile come nello scenario originale)
    ...
    # -----------------------------
    # Carichi edificio (split inflessibile/flessibile) – INVARIATI
    # -----------------------------
    ...
    # -----------------------------
    # Dimensionamento store DSM (da profili flessibili) – INVARIATO
    # -----------------------------
    ...
    # Micro-priorità (ε)
    # -----------------------------
    # Heat Pump (più premiata)
    EPS_HP_BONUS = -1.0e-4

    # Termico (subito dopo HP)
    EPS_TH_DIRECT  = -8.0e-5   # Heat Source -> Building (Thermal Discharge)
    EPS_TH_TES_DIS = -7.0e-5   # TES -> Building (Thermal Dispatch)
    EPS_TH_DSM_CH  = -6.0e-5   # Heat Source -> DSM Heat Store (DSM Heat Charge)
    EPS_TH_TES_CH  = -5.0e-5   # Heat Source -> TES (Thermal Charge)

    # Elettrico (meno premiato del termico)
    EPS_EL_AUTOCONS = -2.0e-5  # PV -> Building (PV Autoconsumo)
    EPS_EL_BAT_DIS  = -1.8e-5  # Battery -> Building
    EPS_EL_DSM_CH   = -1.6e-5  # Building -> DSM Elec Store
    EPS_EL_BAT_CH   = -1.4e-5  # PV -> Battery

    # Export (sfavorito: piccolo costo positivo)
    EPS_EXPORT =  -1.0e-6

    # -----------------------------
    # TERMICO (HP first)
    # -----------------------------
    # HP: elettrico -> calore (efficienza = COP(t) in links_t.efficiency)
    n.add("Link", "Heat Pump",
          bus0="Building Elec", bus1="Heat Source Bus",
          efficiency=1.0,  # COP(t) applicato sotto
          p_nom=heat_capacity*3, p_nom_extendable=False,
          marginal_cost=EPS_HP_BONUS, carrier="heat_pump")

    # 1) Heat Source -> Building (servire subito il calore)
    n.add("Link", "Thermal Discharge",
          bus0="Heat Source Bus", bus1="Building Heat",
          p_nom=heat_capacity, efficiency=1.0,
          marginal_cost=EPS_TH_DIRECT, carrier="heat_distribution")

    # 2) Pre-riscaldo DSM Heat (anticipare)
    n.add("Link", "DSM Heat Charge",
          bus0="Heat Source Bus", bus1="DSM Heat Store",
          p_nom=p_heat_peak, efficiency=1.0,
          marginal_cost=EPS_TH_DSM_CH)
    ...
    eff_store      = max(eta_rt ** 0.5, 1e-6)
    eff_dispatch   = max(eta_rt ** 0.5, 1e-6)

    # e_nom clamp > 0 per evitare KeyError nel post-processing
    n.add("Store", "Battery Store",
          bus="Battery Storage",
          e_nom=max(battery_e_nom, 1e-6),
          e_min_pu=0.15, e_max_pu=0.85,
          e_initial=0.50*max(battery_e_nom, 1e-6),
    ...
    # -----------------------------
    # Time series init (mc & efficiency)
    # -----------------------------
    ...
    # Debug utili
    print("max grid price [EUR/MWh]:", float(pd.Series(n.grid_price_series).max()))
    print("links:", list(n.links.index)[:10], "... tot:", len(n.links.index))
    print("stores:", list(n.stores.index))
    ...
    # Assicura compatibilità con i report
    assert "Battery Store" in n.stores.index, "❌ 'Battery Store' non creato (e_nom > 0)."
    assert "PV Autoconsumo" in n.links.index, "❌ Link 'PV Autoconsumo' mancante."
    assert "Thermal Discharge" in n.links.index, "❌ Link 'Thermal Discharge' mancante."
