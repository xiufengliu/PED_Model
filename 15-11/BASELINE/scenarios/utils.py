...
def load_electricity_price_profile(data_dir, index):
    ...
    Handles multi-year scenarios by matching day-of-year and hour, ignoring the year.
    """
    ...
    try:
        ...
        # 2) Extract the price column (EUR/kWh)
        base_prices = price_df.iloc[:, 0]

        # 3) For multi-year scenarios, match by day-of-year and hour (ignore year)
        # Create a mapping from (month, day, hour) to price
        price_map = {}
        for ts, price in base_prices.items():
            key = (ts.month, ts.day, ts.hour)
            price_map[key] = price

        # 4) Map prices to the requested index
        prices = []
        for ts in index:
            key = (ts.month, ts.day, ts.hour)
            if key in price_map:
                prices.append(price_map[key])
            else:
                # Fallback: use nearest available price or default
                prices.append(50.0 / 1000.0)  # 50 EUR/MWh = 0.05 EUR/kWh

        price_profile = pd.Series(prices, index=index)

        # 5) Convert from EUR/kWh to EUR/MWh (multiply by 1000)
        # Note: prices in CSV are already in EUR/kWh
        # We don't multiply here because invest_opt.py does it

        print(f"Loaded electricity price profile from: {filepath}")
        print(f"Price range: {price_profile.min():.4f} - {price_profile.max():.4f} EUR/kWh")
        return price_profile

    except FileNotFoundError:
        print(f"Warning: Electricity price file not found '{filepath}'. Using default constant price.")
        return pd.Series(0.05, index=index)  # Default: 50 EUR/MWh = 0.05 EUR/kWh
