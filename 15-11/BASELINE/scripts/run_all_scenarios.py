...
def run_all_scenarios(config_file, params_file, output_dir):
    ...
    return results
    # --- Salva risultati orari per plotting ---
    try:
        # Crea un DataFrame con i risultati orari principali
        df_hourly = pd.DataFrame({
            "timestamp": network.snapshots,
            "total_load_kwh": sum(
                network.loads_t.p_set[col] for col in network.loads_t.p_set.columns
                if network.loads.loc[col].carrier == 'electricity'
            ),
            "pv_generation_kwh": network.generators_t.p.get("Existing PV", pd.Series(0.0, index=network.snapshots)),
            "grid_import_kwh": network.generators_t.p.get("Grid", pd.Series(0.0, index=network.snapshots)).clip(lower=0),
            "grid_export_kwh": -network.generators_t.p.get("Grid", pd.Series(0.0, index=network.snapshots)).clip(upper=0)
        })

        # Opzionale: assicurati che non ci siano colonne duplicate
        df_hourly = df_hourly.groupby(df_hourly.columns, axis=1).sum()

        hourly_file = os.path.join(OUTPUT_PATH, f"{scenario_name}_hourly_results.csv")
        df_hourly.to_csv(hourly_file, index=False)
        print(f"Hourly results saved to: {hourly_file}")
    except Exception as e:
        print(f"Could not save hourly results: {e}")
...
def compare_scenarios(results, output_dir):
    ...
    try:
        ...
        from scripts.plotting_utils_pv_battery_1 import setup_plotting_style, plot_scenario_comparison
