...
def calculate_summary(n) -> pd.DataFrame:
    ...
    Summary coerente NET lato building:
      ...
      - Prezzi: guardie di unità (se <10 → presumibilmente EUR/kWh, quindi ×1000 a EUR/MWh).
    """
    import pandas as pd
    ...
    if hasattr(n, "grid_price_series") and n.grid_price_series is not None:
        ...
        # ricava dal generator "Grid" (serie oraria se presente, altrimenti scalare)
        if "Grid" in getattr(Gmc, "columns", []):
        ...
        # normalizza a lato building dividendo per l'efficienza d'ingresso (se <1)
        grid_price_net = (grid_price_gen / max(eta_grid, 1e-12)).astype(float)

    # --- EXPORT PRICE (fallback al grid price) ---
    if hasattr(n, "export_price_series") and n.export_price_series is not None:
        export_price_net = pd.Series(n.export_price_series, index=snaps).astype(float)
    else:
        export_price_net = grid_price_net.copy()

    # --- GUARDIE DI UNITÀ: se media < 10 → probabile EUR/kWh → ×1000 a EUR/MWh ---
    try:
        if float(pd.to_numeric(grid_price_net, errors="coerce").dropna().mean()) < 10.0:
            print("⚠ price_elec appare in EUR/kWh → converto a EUR/MWh (×1000) lato report.")
            grid_price_net = grid_price_net * 1000.0
    except Exception:
        pass

    try:
        if float(pd.to_numeric(export_price_net, errors="coerce").dropna().mean()) < 10.0:
            export_price_net = export_price_net * 1000.0
    except Exception:
        pass
    ...
    # Import NET lato building e GROSS lato sorgente
    imp_net   = (-s_or(p1, "Grid Import")).clip(lower=0.0)
    imp_gross = ( s_or(p0, "Grid Import")).clip(lower=0.0)

    # Export: supporta "Grid Export" o "PV Export"
    ...
    peak_import_el_mw = float(imp_net.max())
    peak_import_el_ts = imp_net.idxmax()

    peak_import_th_mw = float(q_dh_net.max())
    peak_import_th_ts = q_dh_net.idxmax()

    # Prezzo DH NET:
    # 1) se esiste series sul link -> già netta; 2) altrimenti marginal_cost del generator "Heating" (params),
    #    SENZA dividere per efficienza.
    lt_mc = getattr(getattr(n, "links_t", None), "marginal_cost", pd.DataFrame())

    dh_price_net = None
    if "Heat import" in getattr(lt_mc, "columns", []):
        dh_price_link = lt_mc["Heat import"].reindex(snaps)
        if dh_price_link.notna().any() and dh_price_link.fillna(0.0).abs().sum() > 0:
            dh_price_net = dh_price_link.astype(float).fillna(0.0)

    if dh_price_net is None:
        if "Heating" in getattr(Gmc, "columns", []):
...
def run_scenario(scenario_name: str, config_file: str, params_file: str) -> pd.DataFrame:
    ...
    # 1) calcola il summary (contiene già i picchi)
    summary_df = calculate_summary(net)

    # 2) aggiungi Peak Shaving vs baseline (se non è baseline e il file esiste)
    try:
    ...
    # 3) ora salva il summary arricchito
