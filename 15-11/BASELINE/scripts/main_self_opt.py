...
def calculate_summary(network) -> pd.DataFrame:
    """
    Summary NET (lato building/sistema) coerente con i nuovi link:
      - Import elettrico NET = somma di:
          (A) Grid -> Building  : (-p1['Grid Import'])
          (B) Grid -> HP (NET e) : (-p1['Heat Pump Grid']) * (eta_traf / eff_hp_link)
          (C) Grid -> Battery (NET e): (-p1['Battery Grid Charge']) * (eta_traf / eff_batt_link)
      - Costi elettrici: energia NET * prezzo NET (+ eventuale mc link)
      - Export: ricavo da mc del link di export se disponibile, altrimenti fallback a prezzo grid
      - DH: invariato, NET su p1('Heat import')
      - Emissioni: su import elettrico NET e calore DH NET
    """
    import pandas as pd
    ...
    # ---------------------------
    # Efficienza trasformatore (dal link Grid Import)
    # ---------------------------
    eta_traf = 1.0
    if ("Grid Import" in links.index) and ("efficiency" in links.columns):
        try:
            eta_traf = float(links.at["Grid Import", "efficiency"])
        except Exception:
            eta_traf = 1.0
    eta_traf = max(eta_traf, 1e-12)

    # ---------------------------
    # Prezzo Grid: GEN e NET
    # ---------------------------
    if hasattr(n, "grid_price_series") and n.grid_price_series is not None:
        grid_price_gen = pd.Series(n.grid_price_series, index=snaps).astype(float)
    else:
    ...
    grid_price_net = (grid_price_gen / eta_traf).astype(float)   # €/MWh NET dopo trafo
    export_price_gen = grid_price_gen.copy()                      # fallback export

    # ---------------------------
    # Import elettrico NET (A, B, C)
    # ---------------------------
    # (A) Grid -> Building (NET diretto)
    imp_building_net = (-s_or(lt_p1, "Grid Import")).clip(lower=0.0)

    # (B) Grid -> HP (NET elettrico ricavato)
    if "Heat Pump Grid" in getattr(lt_p1, "columns", []):
        eff_hp_link = float(links.at["Heat Pump Grid", "efficiency"]) if ("Heat Pump Grid" in links.index and "efficiency" in links.columns) else 1.0
        eff_hp_link = max(eff_hp_link, 1e-12)
        imp_hp_net_e = ((-lt_p1["Heat Pump Grid"]).reindex(snaps).clip(lower=0.0)) * (eta_traf / eff_hp_link)
    else:
        imp_hp_net_e = pd.Series(0.0, index=snaps)

    # (C) Grid -> Battery (NET elettrico ricavato)
    if "Battery Grid Charge" in getattr(lt_p1, "columns", []):
        eff_batt_link = float(links.at["Battery Grid Charge", "efficiency"]) if ("Battery Grid Charge" in links.index and "efficiency" in links.columns) else 1.0
        eff_batt_link = max(eff_batt_link, 1e-12)
        imp_batt_net_e = ((-lt_p1["Battery Grid Charge"]).reindex(snaps).clip(lower=0.0)) * (eta_traf / eff_batt_link)
    else:
        imp_batt_net_e = pd.Series(0.0, index=snaps)

    # Totale import NET di elettricità
    imp_elec_net = (imp_building_net + imp_hp_net_e + imp_batt_net_e)

    # (solo per reporting, opzionale) componenti singole:
    imp_components = {
        "Grid→Building Import (Net, MWh)": imp_building_net,
        "Grid→HP Import (Net, MWh_e)":     imp_hp_net_e,
        "Grid→Battery Import (Net, MWh_e)":imp_batt_net_e,
    }

    # ---------------------------
    # Export (NET sul p1 del link di export)
    # ---------------------------
    if "Grid Export" in getattr(lt_p1, "columns", []):
        exp_col = "Grid Export"
    elif "PV Export" in getattr(lt_p1, "columns", []):
        exp_col = "PV Export"
    else:
        exp_col = None
    ...
    # Prezzo export da mc link se disponibile (tipico: -prezzo), altrimenti fallback prezzo GEN
    if exp_col:
        exp_link_mc = s_or(lt_mc, exp_col, 0.0).astype(float)
        export_unit_price = (-exp_link_mc).where(exp_link_mc != 0.0, export_price_gen)
    else:
        export_unit_price = export_price_gen

    # ---------------------------
    # Costi elettrici (NET)
    # ---------------------------
    # (A) Grid -> Building: prezzo NET + mc del link
    import_link_mc = s_or(lt_mc, "Grid Import", 0.0).astype(float)
    cost_A = float(((imp_building_net) * (grid_price_net + import_link_mc) * w).sum())

    # (B) Grid -> HP: prezzo NET (+ mc del link se presente)
    hp_link_mc = s_or(lt_mc, "Heat Pump Grid", 0.0).astype(float) if "Heat Pump Grid" in getattr(lt_mc, "columns", []) else pd.Series(0.0, index=snaps)
    cost_B = float(((imp_hp_net_e) * (grid_price_net + hp_link_mc) * w).sum())

    # (C) Grid -> Battery: prezzo NET (+ mc del link se presente)
    batt_link_mc = s_or(lt_mc, "Battery Grid Charge", 0.0).astype(float) if "Battery Grid Charge" in getattr(lt_mc, "columns", []) else pd.Series(0.0, index=snaps)
    cost_C = float(((imp_batt_net_e) * (grid_price_net + batt_link_mc) * w).sum())

    rev_export = float((exp_net * export_unit_price * w).sum())
    electricity_import_cost = cost_A + cost_B + cost_C
    ...
    grid_factor_kg_per_mwh = read_co2_factor("Grid Import", "Grid")  # fattore della rete (fallback ragionevole)
    dh_factor_kg_per_mwh   = read_co2_factor("Heat import", "Heating")

    co2_elec_t = float((imp_elec_net * w).sum()) * grid_factor_kg_per_mwh / 1000.0
    co2_dh_t   = float((q_dh_net   * w).sum())   * dh_factor_kg_per_mwh   / 1000.0
    ...
    # ---------------------------
    total_cost = electricity_import_cost + cost_dh - rev_export
    ...
    out = {
        ...
        "Grid→Building Import (Net, MWh)":           float((imp_building_net * w).sum()),
        "Grid→HP Import (Net, MWh_e)":               float((imp_hp_net_e     * w).sum()),
        "Grid→Battery Import (Net, MWh_e)":          float((imp_batt_net_e   * w).sum()),
        "Total Grid Import (Net, MWh)":              float((imp_elec_net     * w).sum()),
        ...
        "  ├─ Cost Grid→Building (EUR)":             cost_A,
        "  ├─ Cost Grid→HP (EUR)":                   cost_B,
        "  └─ Cost Grid→Battery (EUR)":              cost_C,
