...
def run_scenario(scenario_name: str, config_file: str, params_file: str) -> pd.DataFrame:
    ...
    # --- Peak Shaving vs baseline (elettrico e termico) ---
    try:
        if scenario_name.lower() != "baseline":
            # Path del summary baseline
            base_subdir = config.get("scenarios", {}).get("baseline", {}).get("output_subdir", "scenario_baseline")
            base_dir = Path(data_out_base) / base_subdir
            base_sum_file = base_dir / "baseline_summary.csv"

            if base_sum_file.exists():
                base_sum = pd.read_csv(base_sum_file)
                ...
                print(f"[INFO] Baseline summary non trovato: {base_sum_file} → salto il calcolo dello shaving.")
    except Exception as e:
        print(f"[WARN] Peak shaving non calcolato: {e}")

    # Ora salva il summary ARRICCHITO
